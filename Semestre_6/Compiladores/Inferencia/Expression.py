...
class IfThenElse(Expression):
    """
    This class represents a conditional expression. The semantics of a conditional
    expression, such as "if e0 then e1 else e2" on an environment env is as follows:
    1. Evaluate e0 in the environment env, yielding e0_val
    2. If e0_val is true, evaluate e1 in the environment env
    3. If e0_val is false, evaluate e2 in the environment env
    """
    def __init__(self, exp_cond, exp_then, exp_else):
    ...
    def accept(self, visitor, arg):
        """
        Example:
        >>> e = ifThenElse(Bln(True), Num(42), Num(41))
        >>> ev = EvalVisitor()
        >>> e.accept(ev, {})
        42

        >>> e = ifThenElse(Bln(False), Num(42), Num(41))
        >>> ev = EvalVisitor()
        >>> e.accept(ev, {})
        41
        """
