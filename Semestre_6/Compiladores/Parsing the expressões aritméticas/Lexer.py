...
class Lexer:
    ...
    def getToken(self):
        ...
        # Se for uma string pega todos os seus caracteres
            ...
            bool_text = current_char
            while (self.position < self.length and self.input_string[self.position].isalpha()):
                bool_text += self.input_string[self.position]
                self.position += 1
            if bool_text == "true":
                return Token(bool_text, TokenType.TRU)
            elif bool_text == "false":
                return Token(bool_text, TokenType.FLS)
            elif bool_text == "not":
                return Token(bool_text, TokenType.NOT)
            else:
                raise ValueError(f"Unexpected token: {bool_text}")
