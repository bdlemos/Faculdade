...
vector<int> SimplexAux(){
    ...
    while(!stop){
        ...
        for (int i = n; i < tamanho_real_col_auxiliar-1; i++){
            ...
            // Se não encontrou nenhuma variável que entra na base, entao a solução é otima
        ...
        for (int i = n; i < tamanho_real_col_principal - 1; i++){
            if (tableau2[0][i] < -EPSILON){
                ...
                for (int j = 1; j <= n; j++){
                    ...
                    // Se não encontrou nenhuma variável que sai da base, então a solução é ilimitada
                    if(j==n and var_sai==-1) ilimitado = true, coluna_ilimitada = i;
                }
            }
            // Se não encontrou nenhuma variável que entra na base, entao a solução é otima
            if(i==tamanho_real_col_principal-2 and var_entra == -1){
                stop = true;
                break;
            }
        }

        if (stop or ilimitado)
            break;
...
void recupera_certificado_ilimitado(){
    vector<int> certificado(tamanho_real_col_auxiliar);
    for (int i = 1; i < n+1; i++){
        for(int j = n; j < n+m; j++){
