...
vector<int> SimplexAux(){
    ...
    while(!stop){
        ...
        for (int i = n; i < tamanho_real_col_auxiliar-1; i++){
            ...
            if(i==tamanho_real_col_auxiliar-2 and var_entra == -1){
                stop = true;
                break;
            };
        }
    ...
    for (int i = n+m, var = n; i < tamanho_real_col_auxiliar-1; i++){
        ...
        if(base_hot[i] == 1) base_hot[i] = 0, base_hot[var] = 1;
    }
...
void SimplexPrincipal(){
    ...
    set<int> lines;
    for(int i = 1; i <= n; i++) lines.insert(i);
    
    //Forma canonica.
    for(int i = 0; i < base_inicial.size()-1; i++) {
        for(auto it = lines.begin(); it != lines.end(); it++){
            if(base_inicial[i] == 1 and tableau2[*it][i] != 0){
                pivotTableau(*it,i,2);
                lines.erase(it);
                break;
            }
    ...
    while(!stop){
        ...
        for (int i = n; i < tamanho_real_col_principal-1; i++){
            ...
            if(tableau2[0][i] < -EPSILON){
                ...
                break;
            } else if(i==tamanho_real_col_principal-2 and var_entra == -1){
                stop = true;
                break;
            };
...
void recupera_certificado_otimo(){
    ofstream outfile("certificado.txt");
    outfile << "otimo" << endl;
    for (int i = 0; i < n; i++){
        cout << tableau2[0][i] << ' ';
        outfile << tableau2[0][i] << ' ';
    }
    outfile.close();
}
...
void recupera_certificado_inviavel(){
    ofstream outfile("certificado.txt");
    outfile << "inviavel" << endl;

    for (int i = 0; i < n; i++){
        cout << -tableau1[0][i] << ' ';
        outfile << -tableau1[0][i] << ' ';
    }
    outfile.close();
}
...
void recupera_certificado_ilimitado(){
    ofstream outfile("certificado.txt");
    outfile << "ilimitada" << endl;

    vector<double> certificado(tamanho_real_col_auxiliar);
    for (int i = 1; i < n+1; i++){
        for(int j = n; j < n+m; j++){
