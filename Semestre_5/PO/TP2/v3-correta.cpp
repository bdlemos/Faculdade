...
vector<int> SimplexAux(){
    ...
    while(true){
        ...
        // Se não encontrou a variavel que entra, então a solução é ótima
    ...
    while(true){
        ...
        for (int i = n; i < tamanho_real_col_principal - 1; i++){
            if (tableau2[0][i] < -EPSILON){
                var_entra = i;
                // Encontra a linha e a variavel que sai
                for (int j = 1; j <= n; j++){
                    ...
                    // Se não encontrou a variavel que sai, então a solução é ilimitada
        ...
        // Se não encontrou a variavel que entra, então a solução é ótima.
        // Se for iliimitada, então a solução é ilimitada
