...
// Saida: opt
{
     ...
     while ((c = getopt(argc, argv, "isnd:p:lh")) != EOF)
       switch(c) {
         case 'i':
...
// Saida: depende da operacao escolhida
{
  ...
  switch (opt.opescolhida){
   ...
   defineFaseMemLog(0);
         criaVetor(&a,opt.tam,0); inicializaVetorAleatorio(&a);
         criaVetor(&b,opt.tam,1); inicializaVetorAleatorio(&b);
         criaVetor(&c,opt.tam,2); inicializaVetorNulo(&c);
	  defineFaseMemLog(1);
        //printf("Vetor a:\n");
	      acessaVetor(&a);
        //printf("Vetor b:\n");
